...
**حالة المشروع ونقطة التسليم: [`حالة-المشروع.md`](حالة-المشروع.md)** — ابدأ منه.

دليل التشغيل والأعطال: [`دليل-التشغيل.md`](دليل-التشغيل.md)
...
- أول كتاب حقيقي ✓ — 569 صفحة بلا أخطاء تقنية؛ المحرك الافتراضي صار **Tesseract** (يحافظ على مسافات الكلمات).
- مراجعة آلية ✓ — `مراجعة.md` لكل كتاب يرتّب الصفحات الأقل ثقةً؛ `convert.bat` للسحب والإفلات اليومي.
...
## نسخة Google Drive: المنصة كمجلد (مجاني، بلا خادم)

أبسط مسار على الهاتف: أداة Apps Script تعمل داخل حسابك — تضع PDF في مجلد
«كتب-للتحويل» فتجد النتيجة في «كتب-محوّلة» (صفحة صفحة + `book.md` + `metadata.json`)
بمحرك OCR الخاص بـGoogle (جودة عربية قوية).

**التثبيت (مرة واحدة):**

1. افتح <https://script.google.com> ← **New project**.
2. احذف الكود الافتراضي والصق محتوى [`deploy/google-drive-ocr.gs`](deploy/google-drive-ocr.gs) كاملًا ← احفظ.
3. من الشريط الجانبي **Services (+)** ← اختر **Drive API** ← الإصدار **v2** والمعرّف `Drive` ← **Add**.
4. من قائمة الدوال أعلى المحرر اختر `setup` ← **Run** ← وافق على الأذونات.

**الاستخدام:** ارمِ أي PDF في مجلد «كتب-للتحويل» من تطبيق Drive. تابع «الحالة.txt»
داخل مجلد الكتاب. الحصة اليومية للمشغّلات (~90 دقيقة) تكفي كتابًا كبيرًا واحدًا يوميًا تقريبًا.
...
في المرات التالية: نقرة على `start.bat` فقط — أو **اسحب ملفات PDF وأفلتها على `convert.bat`** للتحويل المباشر بلا واجهة.

يلزم أيضًا Tesseract مع حزمة العربية (مرة واحدة): <https://github.com/UB-Mannheim/tesseract/wiki> ثم أضف `C:\Program Files\Tesseract-OCR` إلى PATH — التفاصيل في الدليل.
